...
const allKeys = permissionsGroups.flatMap((group) =>
  group.permissions.map((permission) => permission.key),
);

export const defaultRoleMatrix: Record<string, Record<string, boolean>> = {
  superadmin: Object.fromEntries(allKeys.map((key) => [key, true])),
  guest: Object.fromEntries(allKeys.map((key) => [key, false])),
  admin: {
    'dashboard.view': true,
    'user.view': false,
    'user.create': false,
