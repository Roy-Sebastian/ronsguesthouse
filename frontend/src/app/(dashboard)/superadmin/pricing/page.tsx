...
export default function AdminPricingPage() {
  ...
  const applyBulkPrice = async () => {
    ...
    try {
      ...
      showToast('Harga override berhasil disimpan');
    } catch (err) {
  ...
  const deleteSelectedOverrides = async () => {
    ...
    try {
      ...
      await fetchPrices();
      setSelectedDates(new Set());
      showToast('Override harga berhasil direset');
    } catch {
      await swal.fire({ icon: 'error', title: 'Gagal', text: 'Gagal menghapus harga' });
    }
    setSaving(false);
                ...
                {selectedDates.size > 0 && (
                  <>
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 text-xs font-semibold bg-primary/10 text-primary hover:bg-primary/20 rounded-lg transition-colors border border-primary/30 flex items-center gap-1"
                    >
                      <Save size={12} /> Set Harga ({selectedDates.size} tanggal)
                    </button>
                    <button
                      onClick={deleteSelectedOverrides}
                      className="px-3 py-1.5 text-xs font-semibold bg-red-50 text-red-700 hover:bg-red-100 rounded-lg transition-colors border border-red-200 flex items-center gap-1"
                    >
