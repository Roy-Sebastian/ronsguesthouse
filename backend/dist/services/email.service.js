...
exports.sendBookingRecovery = exports.sendBookingConfirmation = void 0;
...
const sendBookingConfirmation = async (data) => {
    if (!process.env.RESEND_API_KEY) {
        logger_1.logger.warn('Skipping booking confirmation email; RESEND_API_KEY is missing');
        return;
    }
    ...
    try {
      ...
      `,
        ...
        if (response.error) {
            logger_1.logger.error('Resend API failed to send confirmation email', { error: response.error });
        }
        else {
            logger_1.logger.info('Booking confirmation email sent', { bookingCode: data.bookingCode, email: data.email, id: response.data?.id });
        }
    }
    catch (error) {
        logger_1.logger.error('Unexpected error sending confirmation email', { error: error?.message });
    }
};
exports.sendBookingConfirmation = sendBookingConfirmation;
const sendBookingRecovery = async (email, reservations) => {
    if (!process.env.RESEND_API_KEY) {
        logger_1.logger.warn('Skipping booking recovery email; RESEND_API_KEY is missing');
        return;
    }
    try {
    ...
    `).join('');
      ...
      `,
        ...
        if (response.error) {
            logger_1.logger.error('Resend API failed to send recovery email', { error: response.error });
        }
        else {
            logger_1.logger.info('Booking recovery email sent', { email, count: reservations.length });
        }
    }
    catch (error) {
        logger_1.logger.error('Unexpected error sending recovery email', { error: error?.message });
    }
};
exports.sendBookingRecovery = sendBookingRecovery;
